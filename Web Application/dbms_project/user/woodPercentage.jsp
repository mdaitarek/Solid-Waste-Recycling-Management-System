...
try
{
	PreparedStatement ps = conn.prepareStatement("SELECT CONCAT(SUM(process_collected_waste), ' kg'), CONCAT(SUM(process_recycled_amount),' kg'),CONCAT(CAST(SUM(process_recycled_amount)/SUM(process_collected_waste)*100 AS DECIMAL(10,2)), '%'), TO_CHAR(process_date,'YYYY') FROM Recycling_Process JOIN wood_recycling USING (process_id,process_date) GROUP BY TO_CHAR(process_date,'YYYY')");
	ResultSet rs = ps.executeQuery();
	String str = "<center><table border=2><tr><th>COLLECTED WAST</th><th>RECYCLED WASTE</th><th>RECYCLING RATE</th><th>YEAR</th></tr></center>";
	while(rs.next())
	{
